...
static void dump_source_info(struct ffmpeg_source *s, const char *input,
		const char *input_format, const char *options)
{
	FF_BLOG(LOG_INFO,
			"settings:\n"
			"\tinput:                   %s\n"
...
static void ffmpeg_source_update(void *data, obs_data_t *settings)
{
	...
	if (is_local_file) {
		...
		input_format = (char *)obs_data_get_string(settings,
				"input_format");
		options = (char *)obs_data_get_string(settings, "options");
		s->is_looping = false;
	...
	s->options = options ? bstrdup(options) : NULL;
#ifndef __APPLE__
	...
	dump_source_info(s, input, input_format, options);
